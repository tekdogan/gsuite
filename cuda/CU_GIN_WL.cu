...
float* GINLayer(float* edgeIndex, float* featureTensor, float *aggregationVar, float epsilon, int numOfNodes,
	...
	//memcpy(edgeSources, edgeIndex, numOfEdges*sizeof(int));
	for(int i = 0; i<numOfEdges; i++)
	{
		edgeSources[i] = edgeIndex[i];
	}
	
	// index select
	float *indexSelectOutput = (float*)calloc(numOfEdges*outputSize, sizeof(float));
	indexSelectOutput = index_select(featureTensor, numOfNodes, outputSize, 0, edgeSources, numOfEdges, indexSelectOutput);
	
	// epsilon multiplication
	// TODO: add weight constant to params
	float *h_outputLinear = (float*)calloc(numOfNodes*outputSize, sizeof(float));
	linear(indexSelectOutput, numOfNodes, outputSize,
               h_outputLinear, numOfNodes, outputSize);
	
	return h_outputLinear;

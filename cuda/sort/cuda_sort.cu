...
void gpu_qsort(int *data, int n, int edgeSize){
    ...
    // Allocate GPU memory.
    cudaMalloc((void**)&gpuData,2*n*sizeof(int));
    cudaMemcpy(gpuData,data, 2*n*sizeof(int), cudaMemcpyHostToDevice);
    ...
    // Copy back
    cudaMemcpy(data,gpuData, 2*n*sizeof(int), cudaMemcpyDeviceToHost);
